...
use crate::{Boolean2DArray, Boolean3DArray, BooleanArray, Uint8Array};
...
impl<const N: usize, F: PrimeField> Default for Boolean2DArray<N, F> {
    fn default() -> Self {
        let mut array = Vec::with_capacity(N);
        for _ in 0..N {
            let mut row = Vec::with_capacity(N);
            for _ in 0..N {
                row.push(Boolean::<F>::constant(false));
            }
            array.push(row.try_into().unwrap_or_else(|v: Vec<_>| {
                panic!("Expected a Vec of length {} but it was {}", N, v.len())
            }));
        }
        Boolean2DArray(array.try_into().unwrap_or_else(|v: Vec<_>| {
            panic!("Expected a Vec of length {} but it was {}", N, v.len())
        }))
    }
}
