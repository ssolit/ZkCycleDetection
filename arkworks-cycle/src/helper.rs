...
// Takes in an adjacency matrix and flattens it into a boolean vector
pub fn matrix_flattener<const N: usize, ConstraintF: PrimeField>(
    ...
    for i in 0..N {
        for j in i + 1..N {
            ...
            flattened_matrix.push(transacted.clone());
        }
    }
    Ok(flattened_matrix)
}
...
fn poseidon_parameters_for_test<F: PrimeField>() -> Result<PoseidonParameters<ConstraintF>> {
    let alpha = 17;
    let mds = vec![
        vec![
            F::from_str(
